...
interface ParamLocal{
  ...
  outCancel:(props:{status:boolean}) => void;
  outSave:(props:{status:boolean; objSaveData:any, objEditData:any}) => void;
...
const FormTemplate:React.FC<ParamLocal> = ({children, props, style, final_session, status, edit_data, inDataChange, outDataChange, outBackTo, outCancel, outSave, inConfirmDialog, outConfirmDialog,  ...rest}) => {
  ...
  useEffect(()=>{
    ...
    if (!themeLink){
      ...
      // Artikel-Web -> dari BrowserRouter basename
      // href -> mengarah ke folder public
      themeLink.href = `/Artikel-Web/resources/themes/lara-light-amber/theme.css`;
      ...
      if (Object.keys(globalInvalidInput.current).length > 0){
        ...
        return;
          ...
          setTimeout(()=>{
            ...
            outSave({status:true, objSaveData: {...refDataChange.current}, objEditData:{...refDataEditChange.current}})
