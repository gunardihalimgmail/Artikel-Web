import { AppRegistration, Architecture, Article, Assignment, BarChart, Build, BuildCircle, CellTower, Collections, ConnectedTv, Dashboard, DirectionsCar, DomainAdd, Engineering, HighlightAlt, Inventory, LinkedCamera, List, LocalGasStation, Newspaper, People, PlayCircle, QueryStats, Receipt, Settings, VideoStable } from '@mui/icons-material'
    ...
    if (Array.isArray(arrPath)){
      if (arrPath.length > 0){
        arrPath.forEach((obj, idx)=>{
          ...
          else if (obj?.['key'] == 'Article'){
            
            arrFinalTemp = [...arrFinalTemp, <Link key={idx} underline='none' sx={{display:'flex', alignItems:'center', color:'grey', fontSize:'14px'}}><Article sx={{mr: 0.7, fontSize: '18px', color:'grey'}}/> {obj?.['value']}</Link>];
          }
          else if (obj?.['key'] == 'List'){
            
            arrFinalTemp = [...arrFinalTemp, <Link key={idx} underline='none' sx={{display:'flex', alignItems:'center', color:'grey', fontSize:'14px'}}><List sx={{mr: 0.7, fontSize: '18px', color:'grey'}}/> {obj?.['value']}</Link>];
          }
          else if (obj?.['key'] == 'Detail'){
            
            arrFinalTemp = [...arrFinalTemp, <Link key={idx} underline='none' sx={{display:'flex', alignItems:'center', color:'grey', fontSize:'14px'}}><Newspaper sx={{mr: 0.7, fontSize: '18px', color:'grey'}}/> {obj?.['value']}</Link>];
          }
        })
